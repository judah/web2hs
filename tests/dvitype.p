{3:}program DVItype(dvifile,output);label{4:}9999,30;
{:4}const{5:}maxfonts=100;maxwidths=10000;linelength=79;
terminallinelength=150;stacksize=100;namesize=1000;namelength=50;
{:5}type{8:}ASCIIcode=32..126;{:8}{9:}textfile=packed file of char;
{:9}{21:}eightbits=0..255;bytefile=packed file of eightbits;
{:21}var{10:}xord:array[char]of ASCIIcode;xchr:array[0..255]of char;
{:10}{22:}dvifile:bytefile;tfmfile:bytefile;{:22}{24:}curloc:integer;
...
tfmchecksum:integer;tfmdesignsize:integer;tfmconv:real;
...
{:42}{45:}buffer:array[0..terminallinelength]of ASCIIcode;
termin:textfile;termout:textfile;{:45}{48:}bufptr:0..terminallinelength;
{:48}{57:}inpostamble:boolean;
{:57}{64:}defaultdirectory:packed array[1..9]of char;
...
firstbackpointer:integer;startloc:integer;afterpre:integer;
{:101}{108:}k,m,n,p,q:integer;{:108}procedure initialize;var i:integer;
begin writeln('This is DVItype, Version 3.6');
...
{:58}{65:}defaultdirectory:='TeXfonts:';{:65}{68:}textptr:=0;
{:68}{74:}maxv:=2147483548;maxh:=2147483548;maxs:=stacksize+1;
maxvsofar:=0;maxhsofar:=0;maxssofar:=0;pagecount:=0;
{:74}{98:}oldbackpointer:=-1;started:=false;{:98}end;
{:3}{7:}procedure jumpout;begin goto 9999;end;
{:7}{23:}procedure opendvifile;begin reset(dvifile);curloc:=0;end;
procedure opentfmfile;begin reset(tfmfile,curname);end;
{:23}{26:}procedure readtfmword;begin read(tfmfile,b0);read(tfmfile,b1);
read(tfmfile,b2);read(tfmfile,b3);end;
{:26}{27:}function getbyte:integer;var b:eightbits;
...
begin if f=maxfonts then write('UNDEFINED!')else begin for k:=fontname[f
...
goto 9999;9997:writeln('---not loaded, TFM file is bad');
...
var k:0..terminallinelength;begin break(termout);reset(termin);
if eoln(termin)then readln(termin);k:=0;
while(k<terminallinelength)and not eoln(termin)do begin buffer[k]:=xord[
termin^];k:=k+1;get(termin);end;buffer[k]:=32;end;
...
begin rewrite(termout);writeln(termout,'This is DVItype, Version 3.6');
{51:}1:write(termout,'Output level (default=4, ? for help): ');
outmode:=4;inputln;
if buffer[0]<>32 then if(buffer[0]>=48)and(buffer[0]<=52)then outmode:=
buffer[0]-48 else begin write(termout,'Type 4 for complete listing,');
write(termout,' 0 for errors and fonts only,');
writeln(termout,' 1 or 2 or 3 for something in between.');goto 1;
end{:51};{52:}2:write(termout,'Starting page (default=*): ');
...
end else if buffer[bufptr]=32 then startvals:=k else begin write(termout
,'Type, e.g., 1.*.-5 to specify the ');
writeln(termout,'first page with \count0=1, \count2=-5.');goto 2;end;
until startvals=k{:52};
{53:}3:write(termout,'Maximum number of pages (default=1000000): ');
maxpages:=1000000;inputln;bufptr:=0;
if buffer[0]<>32 then begin maxpages:=getinteger;
if maxpages<=0 then begin writeln(termout,
'Please type a positive number.');goto 3;end;end{:53};
{54:}4:write(termout,'Assumed device resolution');
write(termout,' in pixels per inch (default=300/1): ');
resolution:=300.0;inputln;bufptr:=0;
if buffer[0]<>32 then begin k:=getinteger;
...
end else begin write(termout,'Type a ratio of positive integers;');
writeln(termout,' (1 pixel per mm would be 254/10).');goto 4;end;
end{:54};{55:}5:write(termout,
'New magnification (default=0 to keep the old one): ');newmag:=0;
inputln;bufptr:=0;
if buffer[0]<>32 then if(buffer[0]>=48)and(buffer[0]<=57)then newmag:=
getinteger else begin write(termout,
'Type a positive integer to override ');
writeln(termout,'the magnification in the DVI file.');goto 5;end{:55};
{56:}writeln('Options selected:');write('  Starting page = ');
...
1:writeln(' (terse)');2:writeln(' (mnemonics)');3:writeln(' (verbose)');
4:if true then writeln(' (the works)')else begin outmode:=3;
writeln(' (the works: same as level 3 in this DVItype)');end;end;
writeln('  Resolution = ',resolution:12:8,' pixels per inch');
if newmag>0 then writeln('  New magnification factor = ',newmag/1000:8:3
){:56};end;{:50}{59:}procedure definefont(e:integer);var f:0..maxfonts;
p:integer;n:integer;c,q,d,m:integer;r:0..namelength;j,k:0..namesize;
...
begin if f<nf then writeln('---this font was already defined!');
end else begin if f=nf then writeln(
'---this font wasn''t loaded before!');end;
if f=nf then{62:}begin{66:}for k:=1 to namelength do curname[k]:=' ';
if p=0 then begin for k:=1 to 9 do curname[k]:=defaultdirectory[k];r:=9;
...
curname[r+3]:='F';curname[r+4]:='M'{:66};opentfmfile;
if eof(tfmfile)then write('---not loaded, TFM file can''t be opened!')
else begin if(q<=0)or(q>=134217728)then write(
'---not loaded, bad scale (',q:1,')!')else if(d<=0)or(d>=134217728)then
write('---not loaded, bad design size (',d:1,')!')else if inTFM(q)then
{63:}begin fontspace[nf]:=q div 6;
if(c<>0)and(tfmchecksum<>0)and(c<>tfmchecksum)then begin writeln(
'---beware: check sums do not agree!');
writeln('   (',c:1,' vs. ',tfmchecksum:1,')');write('   ');end;
if abs(tfmdesignsize-d)>2 then begin writeln(
'---beware: design sizes do not agree!');
writeln('   (',d:1,' vs. ',tfmdesignsize:1,')');write('   ');end;
...
if outmode=0 then writeln(' ');
end{:62}else{60:}begin if fontchecksum[f]<>c then writeln(
'---check sum doesn''t match previous definition!');
if fontscaledsize[f]<>q then writeln(
'---scaled size doesn''t match previous definition!');
if fontdesignsize[f]<>d then writeln(
...
j:=j+1;k:=k+1;end;end;if mismatch then writeln(
...
textptr:=0;end;end;{:69}{70:}procedure outtext(c:ASCIIcode);
...
then begin flushtext;showing:=true;write(a:1,': ','fntnum',p:1);end;
...
end{:87};247:begin if not showing then begin flushtext;showing:=true;
...
if showing then if outmode>2 then begin write(' v:=',v:1);
...
if showing then if outmode>2 then begin write(' current font is ');
...
p:=firstpar(o);if eof(dvifile)then begin write(' ','Bad DVI file: ',
'the file ended prematurely','!');jumpout;end;
{81:}if o<128 then{88:}begin if(o>32)and(o<=126)then begin outtext(p);
if outmode>1 then begin showing:=true;write(a:1,': ','setchar',p:1);end;
...
flushtext;showing:=true;write(a:1,': ','set',o-127:1,' ',p:1);end;
...
showing:=true;
write(a:1,': ','DVItype capacity exceeded (stack size=',stacksize:1,')')
;
end else write(' ','DVItype capacity exceeded (stack size=',stacksize:1,
...
{:84}others:if specialcases(o,p,a)then goto 30 else goto 9998 end{:81};
...
printfont(curfont);if curfont<>maxfonts then write('!');end;
...
if outmode>2 then if(p<=0)or(q<=0)then write(' (invisible)')else write(
' (',rulepixels(p):1,'x',rulepixels(q):1,' pixels)');end;
if o=137 then goto 30;if showing then if outmode>2 then writeln(' ');
...
if showing then if outmode>2 then begin write(' h:=',h:1);
...
45:{93:}if showing then if outmode>2 then begin writeln(' ');
write('level ',ss:1,':(h=',h:1,',v=',v:1,',w=',w:1,',x=',x:1,',y=',y:1,
',z=',z:1,',hh=',hh:1,',vv=',vv:1,')');end;goto 30{:93};
30:if showing then writeln(' ');end{:80};9998:writeln('!');
dopage:=false;9999:end;{:79}{95:}{99:}procedure scanbop;var k:0..255;
begin repeat if eof(dvifile)then begin write(' ','Bad DVI file: ',
'the file ended prematurely','!');jumpout;end;k:=getbyte;
if(k>=243)and(k<247)then begin definefont(firstpar(k));k:=138;end;
until k<>138;
if k=248 then inpostamble:=true else begin if k<>139 then begin write(
' ','Bad DVI file: ','byte ',curloc-1:1,' is not bop','!');jumpout;end;
newbackpointer:=curloc-1;pagecount:=pagecount+1;
for k:=0 to 9 do count[k]:=signedquad;
if signedquad<>oldbackpointer then writeln('backpointer in byte ',curloc
-4:1,' should be ',oldbackpointer:1,'!');oldbackpointer:=newbackpointer;
...
{96:}repeat if eof(dvifile)then begin write(' ','Bad DVI file: ',
...
,'illegal command at byte ',curloc-1:1,'!');jumpout;end;others:end;
until k=140;{:96};bopseen:=false;end;9999:end;
{:95}{103:}procedure readpostamble;var k:integer;p,q,m:integer;
...
if outmode<4 then{104:}begin if maxv+99<maxvsofar then writeln(
...
write(xchr[getbyte]);end;writeln('''');afterpre:=curloc{:109};
...
if signedquad<>oldbackpointer then writeln('backpointer in byte ',curloc
-4:1,' should be ',oldbackpointer:1,'!');readpostamble;end;
9999:end.{:107}
