...
printesc(505);end else begin printesc(p-257);
...
printint(t);print(983);t:=xovern(mem[r+3].int,1000)*eqtb[5318+t].int;
printscaled(t);if mem[r].hh.b0=1 then begin q:=29998;t:=0;
...
{536:}begin write(logfile,'This is TeX, Version 3.141592');
...
curl:=256;goto 111;
80:{1035:}if curl<256 then begin if mem[curlist.tailfield].hh.b1=
hyphenchar[mainf]+0 then if mem[curq].hh.rh>0 then insdisc:=true;
...
end;goto 60;end;begin mem[curlist.tailfield].hh.rh:=ligstack;
curlist.tailfield:=mem[curlist.tailfield].hh.rh;end{:1036};
100:{1038:}getnext;if curcmd=11 then goto 101;
if curcmd=12 then goto 101;if curcmd=68 then goto 101;xtoken;
if curcmd=11 then goto 101;if curcmd=12 then goto 101;
if curcmd=68 then goto 101;if curcmd=16 then begin scancharnum;
curchr:=curval;goto 101;end;if curcmd=65 then bchar:=256;curr:=bchar;
ligstack:=0;goto 110;101:mains:=eqtb[4751+curchr].hh.rh;
if mains=1000 then curlist.auxfield.hh.lh:=1000 else if mains<1000 then
begin if mains>0 then curlist.auxfield.hh.lh:=mains;
end else if curlist.auxfield.hh.lh<1000 then curlist.auxfield.hh.lh:=
...
>=128 then begin if curl<256 then begin if mem[curlist.tailfield].hh.b1=
hyphenchar[mainf]+0 then if mem[curq].hh.rh>0 then insdisc:=true;
if ligaturepresent then begin mainp:=newligature(mainf,curl,mem[curq].hh
.rh);if lfthit then begin mem[mainp].hh.b1:=2;lfthit:=false;end;
if rthit then if ligstack=0 then begin mem[mainp].hh.b1:=mem[mainp].hh.
...
curmark[c]);storefmtfile;goto 10;printnl(1282);goto 10;end;10:end;
...
fileoffset:=0;{:55}{61:}write(termout,'This is TeX, Version 3.141592');
